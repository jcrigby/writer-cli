...
To start the Writer CLI from the source code (after building), run the following command from the root directory:
...
The integration tests are designed to validate the end-to-end functionality of the Writer CLI. They are not run as part of the default `npm run test` command.
...
- Consult [WRITER.md](https://github.com/google-gemini/gemini-cli/blob/main/WRITER.md) (typically found in the project root) for specific instructions related to AI-assisted development, including conventions for React, comments, and Git usage.
...
1.  **Start the Writer CLI in development mode:**
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop Writer CLI. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.writer/sandbox.Dockerfile` and/or `.writer/sandbox.bashrc` under your project settings directory (`.gemini`) and running `gemini` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
