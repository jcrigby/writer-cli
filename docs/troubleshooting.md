...
- **Q: How do I update Writer CLI to the latest version?**
  - A: If installed globally via npm, update Writer CLI using the command `npm install -g @google/gemini-cli@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are Writer CLI configuration files stored?**
  - A: The CLI configuration is stored within two `settings.json` files: one in your home directory and one in your project's root directory. In both locations, `settings.json` is found in the `.writer/` folder. Refer to [CLI Configuration](./cli/configuration.md) for more details.
...
- **Error: Command not found (when attempting to run Writer CLI).**
  - **Cause:** Writer CLI is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure Writer CLI installation was successful.
